...
type DeferredFreeCallback = fn(*mut i8);

/// Something which knows how to free based on a callback.
trait DeferredFreeExecutor {
    fn free_cb(&self, callback: DeferredFreeCallback, data: *mut i8);
}
...
struct DeferredFreeCell<T: DeferredFree>(Option<T>);
...
impl<T: DeferredFree> DeferredFree for DeferredFreeCell<T> {
